		...
		private void Rotate(Vector3 center, float degrees)
		{
			...
			if (Relative == true)
			{
				...
				if (camera != null)
				{
					...
					Global.curObj.transform.RotateAround(worldReferencePoint, camera.transform.up, degrees);
				}
			}
			else
			{
				Global.curObj.transform.rotation *= Quaternion.AngleAxis(degrees, RotateAxis);
			}
		}
	}
}